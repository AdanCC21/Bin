...
// Agregar registro
SCREENS DrawAdd(int screenWidth, int screenHeight, string arch_name, int *p);
            ...
            if(CheckCollisionPointRec(mouse,next_rec)){
                ...
                if(CheckCollisionPointRec(click,next_rec)){
                    ...
                    if(placeFull== true && passFull == true){
                        ...
                        new_reg(p,arch,place,pass,note);
                        return SHOW;
                    }
                    else
                    {
...
// Eliminar registro
SCREENS DrawDelete(int screenWidth, int screenHeight, string arch_name, int *p){
    // ------------------ Recursos ------------------
    Font fuente = LoadFont("../assets/Fuentes/arialroundedmtbold.ttf");
...
void DrawExit(int screenWidth, int screenHeight){
    ...
    Vector2 ByeV = {(screenWidth/2) - (ByeSize.x /2), (screenHeight / 2) - (ByeSize.y / 2)};
